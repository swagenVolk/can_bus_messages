...
if (can_bus_intro_txt_box != null)  {
  ...
  can_bus_intro_txt_box.value += "\nDue to its low cost, data integrity in noisy environments and ubiquity of hardware transceivers (transmitters/receivers) ";
  can_bus_intro_txt_box.value += "\nand bus analyzers, the CAN bus has successfully permeated the auto industry and has expanded into others such as ";
  can_bus_intro_txt_box.value += "\nindustrial automation and medical equipment. ";
  can_bus_intro_txt_box.value += "\nExamples of CAN bus usage in cars:";
  can_bus_intro_txt_box.value += "\n  - Engine control";
  can_bus_intro_txt_box.value += "\n  - Transmission";
  can_bus_intro_txt_box.value += "\n  - ABS brakes";
  can_bus_intro_txt_box.value += "\n  - Instrument panel";
  ...
  can_bus_intro_txt_box.value += "\n  - Brakes that imperceptibly squeeze water off of discs after driving through puddles";
  can_bus_intro_txt_box.value += "\n  - Passenger side mirror tilts down when car is in reverse to aid in parallel parking";
  can_bus_intro_txt_box.value += "\n  - OBD-II plug-in tool for displaying Diagnostic Trouble Codes (DTCs)";
  can_bus_intro_txt_box.value += "\n  - More devices expected with Advanced Driver Assistance Systems (ADAS)";
  can_bus_intro_txt_box.value += "\n";
...
var goals = document.getElementById ("can_bus_goals_txt_box");

if (goals != null)  {
  goals.value = "Goals of this web page include: ";
  goals.value += "\n\"The best way to learn is to teach.\" The author learning the CAN bus by coding and documenting is the initial goal. ";
  goals.value += "\nUnveil an embedded technology used daily in cars, vending machines, factory automation, medical equipment, stepper motors, etc. ";
  goals.value += "\n    \"You're never more than 10 feet away from a spider.\" The same is regularly true for equipment utilizing the CAN bus. ";
  goals.value += "\n";
  goals.value += "\nEnable users to create their own CAN bus message (frame) without knowing all the underlying details. ";
  goals.value += "\nDeconstruct user created CAN bus frame(s), showing the encapsulated bit fields and their meaning. ";
  goals.value += "\n";
  goals.value += "\nAdditional goals include: ";
  goals.value += "\nHighlight the physical simplicity of the CAN bus wiring that reduces cost and contributes to its popularity. ";
  goals.value += "\nExplain that a serial bus transmitting 1 bit at a time can cover longer distances than a parallel bus. ";
  goals.value += "\nShow that differential signaling provides more robust communications in an electrically noisy environment. ";
  goals.value += "\nIntroduce bit stuffing, which keeps CAN nodes synchronized during frame transmission without a dedicated clock signal. ";
  goals.value += "\n";
  goals.value += "\nFamiliarize readers with the CRC (Cyclic Redundancy Check) field. ";
  goals.value += "\n    The CRC is generated by the sender from its preceding data in the frame, and sent along in its corresponding field. ";
  goals.value += "\n    The receiver calculates the CRC from the frame data it receives prior to the CRC field, using the same algorithm. ";
  goals.value += "\n    If the sender's and receiver's CRCs do not match, that means the sent frame was corrupted during transmission. ";
  goals.value += "\n";
  goals.value += "\nAcquaint readers with the ACK (acknowledgement) field, used by receivers to indicate a mismatched CRC or other problem. ";
  goals.value += "\n    The 2 bits of the ACK field are expected to both be 1s. Even though the sender is driving the signals on the bus, ";
  goals.value += "\n    the receiver can override the senders 1 and drive it to 0 and all the nodes on the bus will see the 0 ";
  goals.value += "\n    and know that there were problems with the frame. ";
